                             ...
                             size_t nbEle, int blockSize)
{
    ...
    unsigned char *output = (unsigned char *)malloc(sizeof(float) + sizeof(float) * nbEle);
            ...
            if (num_remainder_in_rm > 0)
            {
                ...
                for (i = nbEle - num_remainder_in_rm; i < nbEle; i = i + block_size)
                {
                    ...
                    else
                    {
                        ...
                        outSize_perthread++;
                        block_pointer++;
                        signbytelength = convertIntArray2ByteArray_fast_1b_args(temp_sign_arr, num_remainder_in_rm, block_pointer); 
                        block_pointer += signbytelength;
                        outSize_perthread += signbytelength;
                        savedbitsbytelength = Jiajun_save_fixed_length_bits(temp_predict_arr, num_remainder_in_rm, block_pointer, bit_count);
                        block_pointer += savedbitsbytelength;
                        outSize_perthread += savedbitsbytelength;
                    }
                }
