...
{
    // Handle edge case of no work to do.
    if (intArrayLength == 0) {
        return 0;
    }
    ...
    // Common setup for all remainder cases
    size_t main_idx = 0;
    ...
    // The main optimization: Switch outside the loop.
    // Each case now contains a dedicated, highly optimized loop.
    switch(remainder_bit) {
        case 1: {
            for (size_t i = 0; i < intArrayLength; i++) {
                unsigned int val = unsignintArray[i];
                unsigned int tmp_bytes = val >> 1;
                for (unsigned int j = 0; j < byte_count; j++) {
        ...
        default:
            // This case should not be reached since remainder_bit is [0,7]
            // and the 0 case is handled separately above.
            break;
    }
