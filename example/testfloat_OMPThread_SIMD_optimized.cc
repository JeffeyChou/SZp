/**
 *  @file test_omp_chunk_benchmark.cc
 *  @author Jiefeng Zhou, based on benchmark_chunks.cc
 *  @date Nov, 2025
 *  @brief A microbenchmark tool for testing and comparing OpenMP-based SZP functions
 *         (standard vs. SIMD-optimized buffer versions) using pre-chunked binary data.
 */
...
void print_usage() {
    printf("Usage: ./test_omp_chunk_benchmark <data_file.[npy|f32|dat]> <err_bound> <num_chunks> <algo_block_size> [-r reps] [-w warmups]\n");
    ...
    printf("Example:\n");
    printf("  ./test_omp_chunk_benchmark ../node_feat.f32 1e-4 512 1024 -r 50 -w 10\n");
}
...
int main(int argc, char *argv[]) {
    ...
    printf("--- SZP OpenMP Benchmark (SIMD vs. Non-SIMD) ---\n");
    ...
    for (const auto& chunk_size : chunk_sizes_bytes) {
        ...
        if (!compressed_data_nb || !decompressed_data_nb || !compressed_data_b || !decompressed_data_b) {
        ...
        // --- Run Non-Optimized OpenMP Benchmarks ---
        std::vector<double> comp_timings_nb;
        for (int i = 0; i < warmup_runs + repetitions; ++i) {
            cost_start();
            szp_float_openmp_threadblock_arg(compressed_data_nb, original_data, &compressed_size_nb, abs_err_bound, num_elements, algo_block_size);
            cost_end();
        ...
        for (int i = 0; i < warmup_runs + repetitions; ++i) {
            cost_start();
            szp_float_decompress_openmp_threadblock_arg(decompressed_data_nb, num_elements, abs_err_bound, algo_block_size, compressed_data_nb + sizeof(float));
            cost_end();
        ...
        // --- Run SIMD-Optimized OpenMP Benchmarks ---
        std::vector<double> comp_timings_b;
        for (int i = 0; i < warmup_runs + repetitions; ++i) {
            cost_start();
            szp_float_openmp_threadblock_arg_buffer(compressed_data_b, original_data, &compressed_size_b, abs_err_bound, num_elements, algo_block_size);
            cost_end();
        ...
        for (int i = 0; i < warmup_runs + repetitions; ++i) {
            cost_start();
            szp_float_decompress_openmp_threadblock_arg_buffer(decompressed_data_b, num_elements, abs_err_bound, algo_block_size, compressed_data_b + sizeof(float));
            cost_end();
        ...
        // --- Print Unified Table ---
        printf("  %-20s | %18s | %18s | %s\n", "Metric", "Non-Optimized OMP", "SIMD-Optimized OMP", "Notes");
        printf("  --------------------|--------------------|--------------------|-------------------------\n");
